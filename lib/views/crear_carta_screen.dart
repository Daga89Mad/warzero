...
/// Pantalla para crear una carta nueva.
/// Todos los campos del documento `Cartas/{id}` son editables.
/// Al guardar se usa `.add()` para que Firestore asigne un ID automático.
class CrearCartaScreen extends StatefulWidget {
  const CrearCartaScreen({super.key});
...
class _CrearCartaScreenState extends State<CrearCartaScreen> {
  ...
  final _fuerzaCtrl = TextEditingController(text: '1');
  final _defensaCtrl = TextEditingController(text: '0');
  final _costeCtrl = TextEditingController(text: '1');
  final _idHabilidadCtrl = TextEditingController(text: '0');
  final _movimientoCtrl = TextEditingController(text: '1');
  final _evolucionCosteCtrl = TextEditingController(text: '0');

  int _ejercito = 1;
  int _tipo = 1; // 1 Terrestre, 2 Volador, 3 Marino

  // Evolución
  CartaModel? _evolucionCarta; // carta seleccionada como evolución
  ...
  Future<void> _seleccionarEvolucion() async {
    final result = await Navigator.of(context).push<CartaModel>(
      MaterialPageRoute(
        builder: (_) => const SeleccionarCartaScreen(),
      ),
    );
  ...
  Future<void> _guardar() async {
    ...
    final data = <String, dynamic>{
      ...
      'Movimiento': int.tryParse(_movimientoCtrl.text) ?? 1,
      'Tipo': _tipo,
    ...
    try {
      await _db.collection('Cartas').add(data);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Carta creada correctamente',
              style: TextStyle(fontFamily: 'Cinzel')),
          backgroundColor: Color(0xFF1A2A0A),
        ),
      );
                      ...
                      child: _evolucionCarta != null
                                  ...
                                  child: ClipRRect(
                                    ...
                                    child: _evolucionCarta!.imagen.isNotEmpty
                                        ...
                                        : const Icon(Icons.shield_outlined,
                                            size: 14, color: Color(0xFF2A3A4A)),
                                  ),
                                ),
                                const SizedBox(width: 10),
                                Expanded(
                          ...
                          : const Row(
                              children: [
                                ...
                                SizedBox(width: 10),
                                Text(
                                  'Pulsa para elegir carta de evolución',
                                  style: TextStyle(
                                    fontSize: 11,
                                    color: Color(0xFF506070),
                                    fontFamily: 'Cinzel',
                    ...
                    : const Row(
                        ...
                        children: [
                          Icon(Icons.save_outlined,
                              size: 16, color: Color(0xFF4ABB58)),
                          SizedBox(width: 10),
                          Text(
                            'GUARDAR CARTA',
                            style: TextStyle(
  ...
  Widget _buildTextField({
    ...
    return TextFormField(
      ...
      onChanged: (_) {
        // Para refrescar el preview de imagen
